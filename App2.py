...
if "GEMINI_API_KEY" in st.secrets:
    ...
    GEMINI_API_KEY = os.environ.get("GENERATIVE_LANGUAGE_API_KEY", "")
...
def format_remedies(text):
    """Formats remedy text by adding newlines before numbered points.
    Fixes issue where hyphens were causing unwanted newlines.
    """
    if not isinstance(text, str):
        text = str(text)
    
    # Modified regex: Removed the `|[*-]\s*` part.
    # Now, newlines are only inserted before patterns like "1. ", "2. ", etc.
    formatted = re.sub(r'(\d+\.\s*)', r'\n\1', text.strip())
    
    # Remove leading newline if generated at start
...
def segment_with_unet(image: Image.Image, mask_radius_factor=0.75):
    ...
    for cnt in contours:
        ...
        cv2.circle(circled_img, center, radius, (0, 0, 255), 1) 
...
def predict_disease_with_gemini(image_pil: Image.Image) -> str:
    ...
    """
    if GEMINI_API_KEY == "" and "GENERATIVE_LANGUAGE_API_KEY" not in os.environ:
    ...
    if uploaded_image is not None:
        ...
        if st.button("🔍 Analyze Image"):
            ...
            if predicted_disease not in ["API Key Missing", "API Error", "Parse Error", "Error", "I don't know", "Not a plant image"]:
                ...
                st.image(segmented_image_display, caption="Detected Infections (Circles show detected infections)", use_column_width=True)

                with st.spinner('Fetching AI-generated remedies...'):
                ...
                if remedies:
                    st.markdown("### 💊 AI-Generated Disease Remedies & Care")
