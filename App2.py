...
bg_image = get_base64_image("Img_130716.png") # Assuming Img_130716.png is the new correct path
...
def format_remedies(text):
    ...
    It first removes all existing internal newlines and extra spaces to flatten the text,
    then re-inserts newlines only at the start of new numbered points.
    """
    if not isinstance(text, str):
        text = str(text)
    
    # Step 1: Replace any sequence of whitespace (including newlines) with a single space.
    # This aggressively flattens the text and removes problematic internal newlines.
    ...
    # This regex ensures we catch patterns like "1. ", "2. ", etc., and ensures
    # the newline is inserted correctly without affecting existing content.
...
def predict_disease_with_gemini(image_pil: Image.Image) -> str:
    ...
    # Modified vision_prompt to request Bengali translation
    vision_prompt = """
        ...
        if result.get("candidates") and len(result["candidates"]) > 0:
            ...
            if content and content.get("parts") and len(content["parts"]) > 0:
                ...
                if isinstance(crops_list, list):
                    ...
                    return valid_crops
        return [{"englishName": "Failed to get crop recommendations from AI.", "bengaliName": "এআই থেকে ফসলের সুপারিশ পেতে ব্যর্থ হয়েছে।"}]
    except requests.exceptions.RequestException as e:
...
def main():
    ...
    if st.button("🌱 Recommend Crops"):
        ...
        if suitable_crops and suitable_crops[0].get("englishName") not in ["API Key Missing", "API Error", "Parse Error", "Error", "Failed to get crop recommendations from AI."]:
            # Format for display: "English Name (Bengali Name)"
            formatted_crops = [f'{crop["englishName"]} ({crop["bengaliName"]})' for crop in suitable_crops]
            st.markdown(f'<div class="crop-recommendation-box"><strong>Recommended Crops:</strong><br>- {"<br>- ".join(formatted_crops)}</div>', unsafe_allow_html=True)
        else:
            # Handle error message for crop recommendations
            st.warning(f"Could not recommend crops: {suitable_crops[0].get('englishName')}. Please try again or check your API key.")
    ...
    if uploaded_image is not None:
        ...
        if st.button("🔍 Analyze Image"):
            ...
            if predicted_disease not in ["API Key Missing", "API Error", "Parse Error", "Error", "I don't know", "Not a plant image"]:
                ...
                # predicted_disease will now directly contain "English (Bengali)"
